...
kapten=base64.b64decode(b'CjxUaGlzIENvZGUgR2VuZXJhdGVkIEJ5IHNleGE+Cg==').decode()
...
class Asm:
    ...
    def main(self):
        ...
        print("successfully write to %s" % pyc_file)

class sexa:
    def __init__(self,code):
        self.kodek=code

    def sexa_py3(self, this_code):
    ...
    def puki(self, this_text, this_code):
        print(this_text)
        self.sexa_py3(this_code)
        return list(this_code.co_consts)

    def main(self):
        print("# This Script Written By Mr. E ")
...
def load_module(Sparator):
    ...
    try:
        ...
        try:
            ...
            exit("There seems to be an error in the file %s"%Sparator)
    return y
...
def menu(sys):
    asm = False
    if len(sys) <= 2:
        exit("usage: sexa3 (sexa|sex) file.py")
    file = sys[2]
    if sys[1] == "sex":
        asm = True
    cekfile(file)
